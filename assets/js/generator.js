jQuery( document ).ready( function ( $ ) {
	...
	$search.on( {
		...
		keyup: function ( e ) {
			...
			$choice.each( function () {
				// Get shortcode name
				var name = $( this ).children( 'strong' ).text(), desc = $( this ).data( 'desc' ), group = $( this ).data( 'group' );
				// Show choice if matched
		...
		$.ajax( {
			...
			success: function ( data ) {
				...
				$( '.su-generator-upload-button input:file' ).each( function () {
					// Prepare data
					var $container = $( this ).parent( 'span' ).parent( 'div' ), $value = $container.find( '.su-generator-upload-field' ).children( 'input:text' );
					$( this ).fileupload( {
						paramName: 'file',
						url: ajaxurl,
						formData: {
				...
				$( '.su-generator-switch' ).click( function ( e ) {
					// Prepare data
					var $switch = $( this ), $value = $switch.parent().children( 'input' ), is_on = !!($value.val() ===
						'yes');
					// Disable
					if ( is_on ) {
						// Change class
				...
				$( 'select#su-generator-attr-taxonomy' ).on( 'change', function () {
					var $taxonomy = $( this ), taxonomy = $taxonomy.val(), $terms = $( 'select#su-generator-attr-tax_term' );
					// Load new options
	...
	function su_generator_parse() {
		// Prepare data
		var query = $selected.val(), prefix = $prefix.val(), $settings = $( '#su-generator-settings .su-generator-attr' ), content = $( '#su-generator-content' ).val(), result = new String( '' );
		// Open shortcode
		result += '[' + prefix + query;
		...
		$settings.each( function () {
			...
			if ( value == null ) value = '';
				...
				if ( value !== '' ) result += ' ' + $( this ).attr( 'name' ) + '="' + $( this ).val() + '"';
		} );
		// End of opening tag
		result += ']';
	...
	function su_generator_update_preview() {
		// Prepare data
		var $preview = $( '#su-generator-preview' ), shortcode = su_generator_parse(), previous = $result.text();
		// Request new preview
