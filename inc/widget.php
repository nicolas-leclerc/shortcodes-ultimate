	...
	if ( !class_exists( 'Shortcodes_Ultimate_Widget' ) ) {
		...
		class Shortcodes_Ultimate_Widget extends WP_Widget {
			...
			function Shortcodes_Ultimate_Widget() {
				global $shult;
				/* Widget settings. */
				$widget_ops = array( 'classname' => 'shortcodes-ultimate', 'description' => __( 'Special Shortcodes Ultimate widget', $shult->textdomain ) );
				/* Widget control settings. */
				$control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'shortcodes-ultimate' );
				/* Create the widget. */
				$this->WP_Widget( 'shortcodes-ultimate', __( 'Shortcodes Ultimate', $shult->textdomain ), $widget_ops, $control_ops );
			}
			...
			function form( $instance ) {
				...
				$defaults = array( 'title' => __( 'Shortcodes Ultimate', $shult->textdomain ), 'content' => '' );
				$instance = wp_parse_args( ( array ) $instance, $defaults );
				?>
				<!-- Widget Title: Text Input -->
				<p>
					<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:', $shult->textdomain ); ?></label>
