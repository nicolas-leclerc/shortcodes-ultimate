<?php
	...
	function su_heading_shortcode( $atts, $content = null ) {
		...
		return '<div class="su-heading su-heading-style-' . $atts['style'] . ' su-heading-align-' . $atts['align'] . '"><div class="su-heading-inner">' . $content . '</div></div>';
	}
	...
	function su_tabs_shortcode( $atts, $content ) {
		...
		if ( is_array( $GLOBALS['tabs'] ) ) {
			...
			$return = '<div class="su-tabs su-tabs-style-' . $atts['style'] . '"><div class="su-tabs-nav">' . implode( '', $tabs ) . '</div><div class="su-tabs-panes">' . implode( "\n", $panes ) . '</div><div class="su-spacer"></div></div>';
		}
		// Unset globals
	...
	function su_tab_shortcode( $atts, $content ) {
		...
		$x = $GLOBALS['tab_count'];
		$GLOBALS['tabs'][$x] = array( 'title' => sprintf( $atts['title'], $GLOBALS['tab_count'] ), 'content' => do_shortcode( $content ) );
		$GLOBALS['tab_count']++;
	}
	...
	function su_spoiler_shortcode( $atts, $content = null ) {
		global $shult;
		$atts = shortcode_atts( array(
			'title' => __( 'Spoiler title', $shult->textdomain ),
			'open' => 'no',
			'style' => 'default'
			), $atts );
		...
		su_query_asset( 'js', 'jquery' );
		su_query_asset( 'js', 'su-other-shortcodes' );
		return '<div class="su-spoiler su-spoiler-style-' . $atts['style'] . $open . '"><div class="su-spoiler-title"><span class="su-spoiler-icon"></span>' . $atts['title'] . '</div><div class="su-spoiler-content">' . su_do_shortcode( $content, 's' ) . '</div></div>';
	}
	...
	function su_divider_shortcode( $atts, $content = null ) {
		...
		$atts = shortcode_atts( array(
			'style' => 'default',
			'top' => 'yes',
			'text' => __( 'Go to top', $shult->textdomain )
			), $atts );
		$top = ( $atts['top'] === 'yes' ) ? '<a href="#">' . $atts['text'] . '</a>' : '';
		su_query_asset( 'css', 'su-content-shortcodes' );
		return '<div class="su-divider su-divider-style-' . $atts['style'] . '">' . $top . '</div>';
	}
	...
	function su_spacer_shortcode( $atts, $content = null ) {
		...
		su_query_asset( 'css', 'su-content-shortcodes' );
		return '<div class="su-spacer" style="height:' . $atts['size'] . 'px"></div>';
	}
	...
	function su_highlight_shortcode( $atts, $content = null ) {
		...
		su_query_asset( 'css', 'su-content-shortcodes' );
		return '<span class="su-highlight" style="background:' . $atts['background'] . ';color:' . $atts['color'] . '">&nbsp;' . $content . '&nbsp;</span>';
	}
	...
	function su_label_shortcode( $atts, $content = null ) {
		...
		su_query_asset( 'css', 'su-content-shortcodes' );
		return '<span class="su-label su-label-type-' . $atts['type'] . '">' . $content . '</span>';
	}
	...
	function su_quote_shortcode( $atts, $content = null ) {
		...
		su_query_asset( 'css', 'su-box-shortcodes' );
		return '<div class="su-quote su-quote-style-' . $atts['style'] . $cite_class . '"><div class="su-quote-inner">' . do_shortcode( $content ) . $cite . '</div></div>';
	}
	...
	function su_pullquote_shortcode( $atts, $content = null ) {
		...
		su_query_asset( 'css', 'su-box-shortcodes' );
		return '<div class="su-pullquote su-pullquote-style-' . $atts['style'] . ' su-pullquote-align-' . $atts['align'] . '">' . do_shortcode( $content ) . '</div>';
	}
	...
	function su_dropcap_shortcode( $atts, $content = null ) {
		...
		$em = $atts['size'] * 0.5 . 'em';
		su_query_asset( 'css', 'su-content-shortcodes' );
		return '<span class="su-dropcap su-dropcap-style-' . $atts['style'] . '" style="font-size:' . $em . '">' . $content . '</span>';
	}
	...
	function su_row_shortcode( $atts = null, $content = null ) {
		...
		return '<div class="su-row su-row-style-' . $atts['style'] . '">' . su_do_shortcode( $content, 'r' ) . '</div>';
	}
	...
	function su_column_shortcode( $atts, $content = null ) {
		...
		su_query_asset( 'css', 'su-box-shortcodes' );
		return '<div class="su-column su-column-size-' . str_replace( '/', '-', $atts['size'] ) . '"><div class="su-column-inner">' . su_do_shortcode( $content, 'c' ) . '</div></div>';
	}
	...
	function su_list_shortcode( $atts, $content = null ) {
		...
		su_query_asset( 'css', 'su-content-shortcodes' );
		return '<div class="su-list su-list-style-' . $atts['style'] . '">' . su_do_shortcode( $content, 'l' ) . '</div>';
	}
	...
	function su_button_shortcode( $atts, $content = null ) {
		...
		$styles = array(
			...
			'ts_color' => ( $atts['ts_color'] === 'light' ) ? su_hex_shift( $atts['background'], 'lighter', 50 )
		...
		su_query_asset( 'css', 'su-content-shortcodes' );
		return '<a href="' . $atts['url'] . '" class="' . implode( $classes, ' ' ) . '" style="' . implode( $a_css, ';' ) . '" target="_' . $atts['target'] . '"><span style="' . implode( $span_css, ';' ) . '">' . $content . '</span></a>';
	}
	...
	function su_service_shortcode( $atts, $content = null ) {
		global $shult;
		$atts = shortcode_atts( array(
			'title' => __( 'Service title', $shult->textdomain ),
			'icon' => $shult->url . '/assets/images/service.png',
			'size' => 32
			), $atts );
		su_query_asset( 'css', 'su-box-shortcodes' );
		return '<div class="su-service"><div class="su-service-title" style="padding:' . round( ( $atts['size'] - 16 ) / 2 ) . 'px 0 ' . round( ( $atts['size'] - 16 ) / 2 ) . 'px ' . ( $atts['size'] + 15 ) . 'px"><img src="' . $atts['icon'] . '" width="' . $atts['size'] . '" height="' . $atts['size'] . '" alt="' . $atts['title'] . '" /> ' . $atts['title'] . '</div><div class="su-service-content" style="padding:0 0 0 ' . ( $atts['size'] + 15 ) . 'px">' . do_shortcode( $content ) . '</div></div>';
	}
	...
	function su_box_shortcode( $atts, $content = null ) {
		global $shult;
		$atts = shortcode_atts( array(
			'title' => __( 'This is box title', $shult->textdomain ),
			'style' => 'default',
			'box_color' => '#333333',
			'title_color' => '#FFFFFF',
			'radius' => '3'
		...
		su_query_asset( 'css', 'su-box-shortcodes' );
		// Return result
		return '<div class="su-box su-box-style-' . $atts['style'] . '" style="border-color:' . su_hex_shift( $atts['box_color'], 'darker', 20 ) . ';' . $radius . '"><div class="su-box-title" style="background-color:' . $atts['box_color'] . ';color:' . $atts['title_color'] . ';' . $title_radius . '">' . $atts['title'] . '</div><div class="su-box-content">' . su_do_shortcode( $content, 'b' ) . '</div></div>';
	}
	...
	function su_note_shortcode( $atts, $content = null ) {
		...
		$radius = ( $atts['radius'] != '0' ) ? 'border-radius:' . $atts['radius'] . 'px;-moz-border-radius:' . $atts['radius'] . 'px;-webkit-border-radius:' . $atts['radius'] . 'px;'
				: '';
		su_query_asset( 'css', 'su-box-shortcodes' );
		return '<div class="su-note" style="border-color:' . su_hex_shift( $atts['background'], 'darker', 10 ) . ';' . $radius . '"><div class="su-note-inner" style="background-color:' . $atts['background'] . ';border-color:' . su_hex_shift( $atts['background'], 'lighter', 80 ) . ';color:' . $atts['color'] . ';' . $radius . '">' . su_do_shortcode( $content, 'n' ) . '</div></div>';
	}
	...
	function su_youtube_shortcode( $atts, $content = null ) {
		...
		if ( !$id )
			return '<p class="su-error">YouTube: ' . __( 'please specify correct url', $shult->textdomain ) . '</p>';
		// Prepare autoplay
		$autoplay = ($atts['autoplay'] === 'yes') ? '?autoplay=1' : '';
		// Create player
		$return[] = '<div class="su-youtube">';
	...
	function su_vimeo_shortcode( $atts, $content = null ) {
		...
		// Check that url is specified
		if ( !$id )
			return '<p class="su-error">Vimeo: ' . __( 'please specify correct url', $shult->textdomain ) . '</p>';
		// Prepare autoplay
		$autoplay = ($atts['autoplay'] === 'yes') ? '&amp;autoplay=1' : '';
		// Create player
		$return[] = '<div class="su-vimeo">';
	...
	function su_video_shortcode( $atts, $content = null ) {
		...
		// Check that url is specified
		if ( !$atts['url'] )
			return '<p class="su-error">Video: ' . __( 'please specify correct url', $shult->textdomain ) . '</p>';
		// Prepare title
	...
	function su_table_shortcode( $atts, $content = null ) {
		...
		$return = '<div class="su-table su-table-style-' . $atts['style'] . '">';
	...
	function su_permalink_shortcode( $atts, $content = null ) {
		...
		return '<a href="' . get_permalink( $atts['id'] ) . '" title="' . $text . '" target="_' . $atts['target'] . '">' . $text . '</a>';
	}
	...
	function su_members_shortcode( $atts, $content = null ) {
		...
		$atts = shortcode_atts( array(
			'style' => 'default',
			'message' => __( 'This content is for registered users only. Please %login%.', $shult->textdomain ),
			'login' => __( 'login', $shult->textdomain )
			), $atts );
		// Prepare links
		$login = '<a href="' . wp_login_url( get_permalink( get_the_ID() ) ) . '">' . $atts['login'] . '</a>';
		su_query_asset( 'css', 'su-other-shortcodes' );
		return ( is_user_logged_in() && !is_feed() ) ? do_shortcode( $content ) : '<div class="su-members su-members-style-' . $atts['style'] . '"><span class="su-members-inner">' . str_replace( '%login%', $login, $atts['message'] ) . '</span></div>';
	}
	...
	function su_feed_shortcode( $atts, $content = null ) {
		...
		return '<div class="su-feed">' . wp_rss( $atts['url'], $atts['limit'] ) . '</div>';
	}
	...
	function su_document_shortcode( $atts, $content = null ) {
		...
		su_query_asset( 'css', 'su-media-shortcodes' );
		return '<iframe src="http://docs.google.com/viewer?embedded=true&url=' . $atts['url'] . '" width="' . $atts['width'] . '" height="' . $atts['height'] . '" class="su-document"></iframe>';
	}
	...
	function su_gmap_shortcode( $atts, $content = null ) {
		...
		su_query_asset( 'css', 'su-media-shortcodes' );
		return '<iframe width="' . $atts['width'] . '" height="' . $atts['height'] . '" src="http://maps.google.com/maps?q=' . urlencode( $atts['address'] ) . '&amp;output=embed" class="su-gmap"></iframe>';
	}
	...
	function su_slider_shortcode( $atts, $content = null ) {
		...
		// Prepare slides
		$slides = (count( ( array ) $gallery['items'] )) ? $gallery['items'] : false;
		// Slides not found
		if ( !$slides )
			$return = '<p class="su-error">Slider: ' . __( 'images not found', $shult->textdomain ) . '</p>';
		// Slides are found
		else {
			// Open slider
			$return .= '<div id="' . $id . '" class="su-slider su-slider-style-' . $atts['style'] . $centered . ' su-slider-pages-' . $atts['pages'] . '" style="width:' . $atts['width'] . 'px;height:' . $atts['height'] . 'px" data-autoplay="' . $atts['autoplay'] . '" data-speed="' . $atts['speed'] . '" data-mousewheel="' . $mousewheel . '"><div class="su-slider-slides">';
			// Create slides
			foreach ( $slides as $slide ) {
				...
				if ( $atts['title'] === 'yes' && $slide['title'] )
					$title = '<span class="su-slider-slide-title">' . stripslashes( $slide['title'] ) . '</span>';
				// Open slide
				...
				// Slide content without link
					...
					$return .= '<a><img src="' . $slide['image'] . '" alt="' . esc_attr( $slide['title'] ) . '" />' . $title . '</a>';
				// Close slide
				$return .= '</div>';
			}
			// Close slides
	...
	function su_carousel_shortcode( $atts, $content = null ) {
		...
		// Slides not found
		if ( !$slides )
			$return = '<p class="su-error">Carousel: ' . __( 'images not found', $shult->textdomain ) . '</p>';
		// Slides are found
		else {
			// Open slider
			$return .= '<div id="' . $id . '" class="su-carousel su-carousel-style-' . $atts['style'] . $centered . ' su-carousel-pages-' . $atts['pages'] . '" style="width:' . $atts['width'] . 'px;height:' . $atts['height'] . 'px" data-autoplay="' . $atts['autoplay'] . '" data-speed="' . $atts['speed'] . '" data-mousewheel="' . $mousewheel . '" data-items="' . $atts['items'] . '" data-scroll="' . $atts['scroll'] . '"><div class="su-carousel-slides">';
			// Create slides
			foreach ( $slides as $slide ) {
				// Crop the image
				...
				// Prepare slide title
				if ( $atts['title'] === 'yes' && $slide['title'] )
					$title = '<span class="su-carousel-slide-title">' . stripslashes( $slide['title'] ) . '</span>';
				// Open slide
	...
	function su_custom_gallery_shortcode( $atts, $content = null ) {
		...
		// Slides not found
		if ( !$slides )
			$return = '<p class="su-error">Custom gallery: ' . __( 'images not found', $shult->textdomain ) . '</p>';
		// Slides are found
		else {
			// Open gallery
			$return .= '<div class="su-custom-gallery su-custom-gallery-title-' . $atts['title'] . '">';
			// Create slides
			foreach ( $slides as $slide ) {
				// Crop image
				$image = su_image_resize( $slide['image'], $atts['width'], $atts['height'] );
				// Prepare slide title
				if ( $slide['title'] )
					$title = '<span class="su-custom-gallery-title">' . stripslashes( $slide['title'] ) . '</span>';
				// Open slide
				$return .= '<div class="su-custom-gallery-slide">';
				// Slide content with link
				if ( $slide['link'] )
					$return .= '<a href="' . $slide['link'] . '"' . $target . '><img src="' . $image['url'] . '" alt="' . esc_attr( $slide['title'] ) . '" width="' . $atts['width'] . '" height="' . $atts['height'] . '" />' . $title . '</a>';
				// Slide content without link
				else
					$return .= '<a><img src="' . $slide['image'] . '" alt="' . esc_attr( $slide['title'] ) . '" width="' . $atts['width'] . '" height="' . $atts['height'] . '" />' . $title . '</a>';
				// Close slide
				$return .= '</div>';
			}
			// Clear floats
	...
	function su_posts_shortcode( $atts, $content = null ) {
		...
		$ignore_sticky_posts = ( bool ) ( $atts['ignore_sticky_posts'] === 'yes' ) ? true
				: false;
		...
		if ( !empty( $taxonomy ) && !empty( $tax_term ) ) {
			...
			isset( $original_atts['tax_' . $count . '_term'] ) && !empty( $original_atts['tax_' . $count . '_term'] )
				...
				$tax_operator = isset( $original_atts['tax_' . $count . '_operator'] ) ? $original_atts['tax_' . $count . '_operator']
						: 'IN';
				$tax_operator = in_array( $tax_operator, array( 'IN', 'NOT IN', 'AND' ) ) ? $tax_operator
						: 'IN';
		...
		elseif ( file_exists( WP_PLUGIN_DIR . '/' . $shult->slug . '/' . $atts['template'] ) )
			load_template( WP_PLUGIN_DIR . '/' . $shult->slug . '/' . $atts['template'], false );
		// Template not found
		else
			return '<p class="su-error">Posts: ' . __( 'template not found', $shult->textdomain ) . '</p>';
