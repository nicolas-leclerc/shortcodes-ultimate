<?php
	 ...
	 * Generator button
	 */
	function su_generator_button( $page = null, $target = null ) {
		global $shult;
		// Print button
		echo '<a href="#su-generator" class="su-generator-button button" title="' . __( 'Insert shortcode', $shult->textdomain ) . '" data-page="' . $page . '" data-target="' . $target . '"><img src="' . $shult->assets( 'images', 'generator/button.png' ) . '" alt="" /> ' . __( 'Insert shortcode', $shult->textdomain ) . '</a>';
	}
	...
	function su_generator_popup() {
		...
		if ( !in_array( $pagenow, array( 'post.php', 'edit.php', 'post-new.php', 'index.php', 'edit-tags.php', 'widgets.php' ) ) )
			return;
		?>
				...
				<div id="su-generator-header">
					...
					<div id="su-generator-choices">
						...
						foreach ( su_shortcodes() as $name => $shortcode ) {
							$icon = ( $shortcode['icon'] ) ? $shortcode['icon'] : $shult->assets( 'images/icons', $name ) . '.png';
	...
	function su_generator_settings() {
		...
		if ( !current_user_can( 'edit_posts' ) )
			wp_die( __( 'Access denied', $shult->textdomain ) );
		// Param check
		if ( empty( $_REQUEST['shortcode'] ) )
			wp_die( __( 'Shortcode not specified', $shult->textdomain ) );
		// Request queried shortcode
		$shortcode = su_shortcodes( htmlentities( $_REQUEST['shortcode'] ) );
		// Shortcode header
		$return .= '<div id="su-generator-breadcrumbs"><a href="#" id="su-generator-select-another" title="' . __( 'Click to return to the shortcodes list', $shult->textdomain ) . '">' . __( 'All shortcodes', $shult->textdomain ) . '</a> &rarr; <span>' . $shortcode['name'] . '</span> <small>' . $shortcode['desc'] . '</small></div>';
		// Shortcode has atts
			...
			foreach ( $shortcode['atts'] as $attr_name => $attr_info ) {
				...
				switch ( $attr_info['type'] ) {
					...
					case 'select':
						...
						$is_numbers = is_numeric( implode( '', array_keys( $attr_info['values'] ) ) )
								? true : false;
						// Multiple selects
					...
					// Switch
					case 'switch':
						$return .= '<span class="su-generator-switch su-generator-switch-' . $attr_info['default'] . '"><span class="su-generator-yes">' . __( 'Yes', $shult->textdomain ) . '</span><span class="su-generator-no">' . __( 'No', $shult->textdomain ) . '</span></span><input type="hidden" name="' . $attr_name . '" value="' . esc_attr( $attr_info['default'] ) . '" id="su-generator-attr-' . $attr_name . '" class="su-generator-attr" />';
						break;
					// Upload
					case 'upload':
						$return .= '<div class="su-generator-upload-field-wrap"><span class="su-generator-upload-field"><input type="text" name="' . $attr_name . '" value="' . esc_attr( $attr_info['default'] ) . '" id="su-generator-attr-' . $attr_name . '" class="su-generator-attr" /></span><span class="su-generator-upload-button"><a href="#" class="button">&hellip;</a><input type="file" name="su_generator_file_upload" /></span></div><small class="description">' . __( 'Max upload file size', $shult->textdomain ) . ': ' . floor( wp_max_upload_size() / 1024 / 1024 ) . __( 'Mb', $shult->textdomain ) . '. <a href="http://www.wpbeginner.com/wp-tutorials/how-to-increase-the-maximum-file-upload-size-in-wordpress/" target="_blank">' . __( 'How to increase', $shult->textdomain ) . '</a>.</small>';
						break;
					// Color
					case 'color':
					...
					case 'gallery':
						...
						// Galleries not created
							...
							$return .= '<option value="0" selected>' . __( 'Galleries not found', $shult->textdomain ) . '</option>';
						$return .= '</select><small class="description"><a href="' . $shult->admin_url . '#tab-3" target="_blank">' . __( 'Manage galleries', $shult->textdomain ) . '</a></small>';
						break;
					// Number
					case 'number':
				...
				if ( $attr_info['desc'] )
					$return .= '<div class="su-generator-attr-desc">' . str_replace( '<b%value>', '<b class="su-generator-set-value" title="' . __( 'Click to set this value', $shult->textdomain ) . '">', $attr_info['desc'] ) . '</div>';
				$return .= '</div>';
			}
		}
		...
		// Wrapping shortcode
			...
			$return .= '<div class="su-generator-attr-container"><h5>' . __( 'Content', $shult->textdomain ) . '</h5><textarea name="su-generator-content" id="su-generator-content" rows="3">' . esc_attr( $shortcode['content'] ) . '</textarea></div>';
		$return .= '<div id="su-generator-preview"></div>';
		$return .= '<div class="su-generator-actions"><a href="#" class="button button-primary button-large" id="su-generator-insert">' . __( 'Insert shortcode', $shult->textdomain ) . '</a> <a href="#" class="button button-large" id="su-generator-preview-link">' . __( 'Live preview', $shult->textdomain ) . '</a> <a href="#" class="button alignright button-large" id="su-generator-cancel">' . __( 'Close window', $shult->textdomain ) . '</a></div>';
		echo $return;
		exit;
	}
	...
	function su_generator_preview() {
		...
		if ( !current_user_can( 'edit_posts' ) )
			die( __( 'Access denied', $shult->textdomain ) );
		// Output results
